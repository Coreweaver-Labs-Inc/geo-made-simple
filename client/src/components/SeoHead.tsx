...
export function SeoHead({ title, description, path, noIndex = false, ogType = "website", ogImage = DEFAULT_OG_IMAGE, ogImageAlt = DEFAULT_OG_IMAGE_ALT }: { title: string; description: string; path: string; noIndex?: boolean; ogType?: "website" | "article"; ogImage?: string; ogImageAlt?: string }) {
  useEffect(() => {
  ...
  }, [description, noIndex, ogImage, ogImageAlt, ogType, path, title]);
  return null;
}
