...
export type HeadMeta = { title: string; description: string; ogType?: "website" | "article"; ogImage?: string; canonicalPath?: string; publishedTime?: string; noindex?: boolean; notFound?: boolean };
...
export async function prefetchForPath(url: string, queryClient: QueryClient, prefetch: SsrPrefetch): Promise<HeadMeta> {
  ...
  if (clean === "/") return { title: "Coreweaver Labs — Make your brand easier for AI to understand.", description: DESCRIPTION, ogImage: image, canonicalPath: "/" };
  if (clean === "/framework") return { title: "The ARM Framework | Coreweaver Labs", description: "Learn how the ARM Framework connects authority, representation, and measurement to create a clearer brand signal for AI search.", ogImage: "/manus-storage/coreweaver-framework_2cadabec.jpg", canonicalPath: clean };
  if (clean === "/products") return { title: "GEO Signal Products | Coreweaver Labs", description: "Explore Coreweaver Labs' practical GEO signal architecture, citation intelligence, and knowledge systems services.", ogImage: "/manus-storage/coreweaver-products_9a7c53f2.jpg", canonicalPath: clean };
