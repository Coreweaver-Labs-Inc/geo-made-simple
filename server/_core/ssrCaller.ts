...
export async function buildSsrPrefetch(req: Request, res: Response): Promise<SsrPrefetch> {
  ...
  return { insightBySlug: slug => caller.insights.bySlug({ slug }), insightsListPublic: () => caller.insights.listPublic(), caseStudyBySlug: slug => caller.caseStudies.bySlug({ slug }) };
}
